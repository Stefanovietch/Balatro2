...
public static class BlindMethods
{
    ...
    private static BalatroPower CreatePower(BlindType type) => Factory[type]();
    public static bool IsBossBlind(this IBlindPower blindPower) => BossTypes.Contains(blindPower.BlindType);
    ...
    private static BlindType GetRandomBlind(Creature creature, bool isBoss = false, List<BlindType>? exclude = null)
    {
        ...
        return (isBoss ? BossTypes : Types).Where(exclude.Contains).TakeRandom(1, creature.CombatState.RunState.Rng.MonsterAi).First();
    }
    
    public static BalatroPower GetRandomBlindPower(Creature creature, bool isBoss = false, List<BlindType>? exclude = null)
    {
