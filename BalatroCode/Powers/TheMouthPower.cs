...
public class TheMouthPower() : BalatroPower, IBlindPower
{
    ...
    public override bool ShouldPlay(CardModel card, AutoPlayType autoPlayType)
    {
        ...
        if (PlayedCardTypes.TryGetValue(card.Owner, out var types) && !types.Contains(card.Type) &&
            types.Count >= 2) return false;
        return base.ShouldPlay(card, autoPlayType);
    }

    public override Task AfterCardPlayed(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        if (cardPlay.Card.Owner.Character is not Character.Balatro)
        ...
        if (types.Count < 2) types.Add(cardPlay.Card.Type);
        return base.AfterCardPlayed(choiceContext, cardPlay);
    }
}