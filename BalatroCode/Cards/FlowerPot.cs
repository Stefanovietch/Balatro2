    ...
    TargetType.AnyEnemy)
{
    ...
    {
        if (cardSource != this || !HasDifferentCardTypes()) return base.ModifyDamageMultiplicative(target, amount, props, dealer, cardSource);
        return 3;
    }
    ...
    {
        return PileType.Hand.GetPile(this.Owner).Cards.Where(c => !c.Equals(this)).Select(c => c.Type).Distinct().Count() >= 3;
    }
}
