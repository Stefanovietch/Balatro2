    ...
    TargetType.Self)
{
        ...
        CardPlay play)
    {
        var cardPlayEntry = CombatManager.Instance.History.CardPlaysFinished
            .LastOrDefault(c => c.HappenedThisTurn(CombatState) && c.CardPlay.Card is not Blueprint);
    ...
    {
        var card = CombatManager.Instance.History.CardPlaysFinished.LastOrDefault(c =>
            c.HappenedThisTurn(CombatState) && c.CardPlay.Card is not (Blueprint or Brainstorm));
