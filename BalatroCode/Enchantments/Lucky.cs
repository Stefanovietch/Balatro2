...
public class Lucky : CustomEnchantmentModel, IChance
{
    ...
    public override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay? cardPlay)
    {
        ...
        if (this.RollChance(Card.Owner, DynamicVars["Chance"].IntValue))
            await PowerCmd.Apply<VigorPower>(choiceContext, player.Creature, Amount, player.Creature, null);

        if (this.RollChance(Card.Owner, DynamicVars["ChanceMoney"].IntValue))
    ...
    {
        if (power is OopsAll6sPower) ModifyCard();
